...
var PhishingService = class {
  ...
  hasSuspiciousSubdomains(hostname) {
    ...
    return parts.some(
      (part) => suspiciousPatterns.some((pattern) => pattern.test(part))
    );
  }
  isShortUrl(hostname) {
    return SHORT_URL_DOMAINS.includes(hostname.toLowerCase());
  }
  containsSuspiciousKeywords(url) {
  ...
  calculateRiskScore(indicators, url) {
    let score = 0;
  ...
  hasHomographAttack(hostname) {
    ...
    const fullwidthPattern = /[ａ-ｚ]/i;
    const confusingChars = /[0oOQ1lI5sS2zZ6G9gq]/;
    const hasConfusing = confusingChars.test(hostname) && hostname.length > 3;
    const idnPattern = /xn--/;
    const suspiciousUnicode = cyrillicPattern.test(hostname) || greekPattern.test(hostname) || fullwidthPattern.test(hostname);
  ...
  hasSuspiciousPort(port) {
    ...
    const suspiciousPorts = [8080, 8e3, 3e3, 5e3, 9e3, 8443, 8888, 9999, 4444, 31337, 1337, 8008];
...
var PortService = class {
  async scanPorts(target, portRange = "1-1000") {
...
// server/services/file-integrity-service.ts
import fs from "fs/promises";
import path from "path";
import crypto from "crypto";
var FileIntegrityService = class {
  baseline = /* @__PURE__ */ new Map();
  async initializeBaseline(directory, recursive = true) {
  ...
  async checkIntegrity(directory, recursive = true) {
  ...
  async updateBaseline(directory, recursive = true) {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isFile()) {
          ...
          if (!entry.name.startsWith(".") && !["node_modules", "__pycache__", ".git", ".svn"].includes(entry.name)) {
  ...
  getChangeReasons(filePath, changeType) {
    const reasons = [];
    const fileName = path.basename(filePath).toLowerCase();
    const fileExt = path.extname(filePath).toLowerCase();
    const dirPath = path.dirname(filePath).toLowerCase();
  ...
  assessRisk(changes) {
    ...
    const executableChanges = changes.filter(
      (c) => path.extname(c.filePath).toLowerCase() === ".exe" || path.extname(c.filePath).toLowerCase() === ".dll"
    ).length;
    if (executableChanges > 0) {
      suspiciousPatterns.push(`${executableChanges} executable file(s) modified`);
    }
    const systemFileChanges = changes.filter(
      (c) => c.filePath.toLowerCase().includes("system32") || c.filePath.toLowerCase().includes("windows") || c.filePath.toLowerCase().includes("/etc/") || c.filePath.toLowerCase().includes("/bin/")
    ).length;
...
var scanResults = pgTable("scan_results", {
  ...
  // 'password', 'phishing', 'port', 'keylogger', 'file_integrity'
...
var monitoredFiles = pgTable("monitored_files", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
...
var insertMonitoredFileSchema = createInsertSchema(monitoredFiles).pick({
  filePath: true,
  hash: true,
  size: true,
  lastModified: true
...
var fileMonitorRequestSchema = z.object({
...
async function registerRoutes(app2) {
  ...
  app2.post("/api/security/file-integrity-init", [
    body("directory").matches(/^[a-zA-Z0-9\/_.-]+$/).withMessage("Directory path contains invalid characters").isLength({ min: 1, max: 500 }).withMessage("Directory path must be between 1 and 500 characters").trim(),
    body("recursive").optional().isBoolean().withMessage("Recursive must be a boolean value"),
    handleValidationErrors
  ], async (req, res) => {
    try {
  ...
  app2.post("/api/security/file-integrity-check", [
    body("directory").matches(/^[a-zA-Z0-9\/_.-]+$/).withMessage("Directory path contains invalid characters").isLength({ min: 1, max: 500 }).withMessage("Directory path must be between 1 and 500 characters").trim(),
    body("recursive").optional().isBoolean().withMessage("Recursive must be a boolean value"),
    handleValidationErrors
  ], async (req, res) => {
  ...
  app2.post("/api/security/file-integrity-update", [
    body("directory").matches(/^[a-zA-Z0-9\/_.-]+$/).withMessage("Directory path contains invalid characters").isLength({ min: 1, max: 500 }).withMessage("Directory path must be between 1 and 500 characters").trim(),
    body("recursive").optional().isBoolean().withMessage("Recursive must be a boolean value"),
    handleValidationErrors
  ], async (req, res) => {
